...
Widget viewAnemoCharacter({required int index, required var context}) {
  var charData = CharacterData.data;
  return Column(
    children: [
      for (int i = 0; i < index; i++)
        if (charData[i].vision == 'anemo')
          Card(
            child: InkWell(
              onTap: () {
                ...
                children: [
                  Stack(
                    children: [
                      ...
                      Image.asset('assets/images/characters/anemo/${charData[i].slug}.webp', width: 100.0,),
                    ],
                  ),
                  SizedBox(width: 8.0,),
                  Column(
                    children: [
                      Text('${charData[i].name}', style: TextStyle(fontSize: 25.0, color: Colors.greenAccent),),
